(() => {
  ...
  const fmtDate=v=>v?new Date(`${v}T12:00:00`).toLocaleDateString('pt-BR',{day:'2-digit',month:'short',year:'numeric'}):'A definir';
  function summary(c){if(c.resumo)return c.resumo;const level=c.escolaridade&&c.escolaridade!=='A definir'?` de nível ${c.escolaridade.toLowerCase()}`:'';return `${c.nome} é uma oportunidade ${c.area?`da área ${String(c.area).toLowerCase()}`:'em acompanhamento'}${level}. ${c.edital_previsto||statusText[c.status]||'Acompanhe as atualizações oficiais.'}`}
  ...
  function inject(){if(document.querySelector('#contest-detail-modal'))return;document.body.insertAdjacentHTML('beforeend','<div id="contest-detail-modal" class="modal"><div class="modal-card contest-detail-shell"><button class="close" id="contest-detail-close">×</button><div id="contest-detail-content"></div></div></div>');const m=document.querySelector('#contest-detail-modal');document.querySelector('#contest-detail-close').onclick=()=>m.classList.remove('open');m.onclick=e=>{if(e.target===m)m.classList.remove('open')}}
