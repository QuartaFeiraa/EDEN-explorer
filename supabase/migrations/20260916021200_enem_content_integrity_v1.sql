...
select 'enem',area_id,topic_id,prompt,options,difficulty,true,true,'original','RUMO ENEM authorial seed v1.1',tags
from seed s
where not exists (
  select 1 from public.question_bank q
  where q.track_id='enem' and q.prompt=s.prompt
);
...
where not exists (
  select 1 from public.question_solutions existing where existing.question_id=q.id
);
...
from essays e
where not exists (
  select 1 from public.essay_topics t where t.track_id='enem' and t.title=e.title
);
